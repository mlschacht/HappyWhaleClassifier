...
def cm_analysis(y_true, y_pred, title, figsize=(10,10)):
    ...
    ax.set_title(title)
    plt.savefig("ConfusionMatrix3_1")

wandb.init(
    project="BGMP_HappyWhale",
    name="Makayla-model3_1", ##update this with your name
    config={"learning rate":1e-5, # possibly update
        "architecture": "CNN",
        "dataset": "Species",
        "epochs": 3, "batch_size":10}  # possibly update
) 
...
plt.savefig('./model3_1_validation_loss.png') 
plt.close() 

y_pred, y_true = predict(model, test_set)
#cm_analysis(y_true, y_pred, "Confusionmatrix_model3_1.png", labels)
