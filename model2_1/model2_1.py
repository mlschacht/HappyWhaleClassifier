...
def cm_analysis(y_true, y_pred, title, figsize=(10,10)):
    ...
    ax.set_title(title)
    plt.savefig("ConfusionMatrix2_1")

wandb.init(
    project="BGMP_HappyWhale",
    name="Makayla-model2_1", ##update this with your name
    config={"learning rate":1e-3, # possibly update
        "architecture": "CNN",
        "dataset": "Species",
        "epochs": 7, "batch_size":60}  # possibly update
) 
...
plt.savefig('./model2_1_validation_loss.png') 
plt.close() 

y_pred, y_true = predict(model, test_set)
#cm_analysis(y_true, y_pred, "Confusionmatrix_model2_1.png", labels)
