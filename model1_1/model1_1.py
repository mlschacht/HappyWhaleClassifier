...
def cm_analysis(y_true, y_pred, title, figsize=(10,10)):
    ...
    ax.set_title(title)
    plt.savefig("ConfusionMatrix1_1")

wandb.init(
    project="BGMP_HappyWhale",
    name="Makayla-model1_1", ##update this with your name
    config={"learning rate":1e-3, # possibly update
        "architecture": "CNN",
        "dataset": "Species",
        "epochs": 5, 
        "batch_size": 48}  # possibly update
) 
...
plt.savefig('./model1_1_validation_loss.png') 
plt.close() 

y_pred, y_true = predict(model, test_set)
#cm_analysis(y_true, y_pred, "Confusionmatrix_model1_1.png", labels)
